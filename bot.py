...
try:
    ...
    VC_AVAILABLE = True
    print("-> vc_handler imported successfully.")
except Exception as e:
    print(f"-> Failed to import vc_handler: {e}")
    VC_AVAILABLE = False

try:
    from yt_handler import get_youtube_stream_url
    YT_AVAILABLE = True
    print("-> yt_handler imported successfully.")
except Exception as e:
    print(f"-> Failed to import yt_handler: {e}")
    YT_AVAILABLE = False
...
print(f"-> Initializing Pyrogram Client (Using String Session: {bool(STRING_SESSION)})")
...
if VC_AVAILABLE:
    try:
        print("-> Setting up PyTgCalls...")
        call_py = setup_vc_calls(app)
        if call_py:
            print("-> PyTgCalls setup and started successfully!")
        else:
            print("-> PyTgCalls setup returned None.")
    except Exception as e:
        print(f"-> CRITICAL VC SETUP ERROR: {e}")
        call_py = None
