...
try:
    ...
    # PyTgCalls v1.x के सही इम्पोर्ट्स
    from pytgcalls.types.input_stream import AudioPiped, VideoPiped
    VC_AVAILABLE = True
    print("-> PyTgCalls library loaded successfully.")
except Exception as e:
    print(f"-> PyTgCalls load skipped/error: {e}")
...
# Assistant Client with error handling for session string
ass = None
if STRING_SESSION and len(STRING_SESSION) > 10:
    try:
        ...
        print(f"-> Assistant Client init error: {e}")

call_py = None
if VC_AVAILABLE and ass:
    try:
...
async def play_audio(client, message):
    ...
    if not VC_AVAILABLE or not call_py or not ass:
        return await m.edit("Song found, but Voice Chat module or Assistant session is inactive.")
...
async def play_video(client, message):
    ...
    if not VC_AVAILABLE or not call_py or not ass:
        return await m.edit("Video found, but Voice Chat module or Assistant session is inactive.")
...
async def main():
    ...
    if ass:
        try:
            ...
            print(f"Assistant start error (Check STRING_SESSION): {e}")
    if call_py and ass and ass.is_connected:
        try:
            await call_py.start()
        except Exception as e:
            print(f"PyTgCalls start error: {e}")
